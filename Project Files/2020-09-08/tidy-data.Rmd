...
min(mpg$cty)
max(mpg$cty)
mad(mpg$cty) # mean absolute deviation
```
...
The command for this is: Shift + Command + m
including a '.' in the function with %>% will place the value where the 'dot'.' is
...
If we want to put dataframes on top of each other, we can use `rbind()`. This represents binding rows.
...
If you want to merge two dataframes sideways, you can use `cbind()`. This represents columns.
