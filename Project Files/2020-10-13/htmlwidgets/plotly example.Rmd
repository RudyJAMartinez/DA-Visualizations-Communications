...
Let's recreate Hans Rosling's famous visualization. If you have never seen it, check out this long video:
...
```{r}
amzn <- read_csv(here::here('amazon.csv')) %>% 
  mutate(mktval = PRC*SHROUT*1000,
         date2 = as.Date(as.character(date), format = '%Y%m%d')) %>% 
  filter(!is.na(mktval)) %>% #Takes out the N/A values from mkval
...
In order to create a worm, `plotly` needs to create multiple data sets capturing the progress. First data set will effectively have only one observation for the first month. The second data set will have 2 observations, and so on. These are all stacked on top of each other to get a large data set. Note that `amzn` has 156 rows. So, we will get 1 + 2 + 3 + ... + 156 =  157 * 156 / 2 = 12,246 observations.
